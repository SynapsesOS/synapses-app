...
import {
  ...
  Users, CheckSquare, DollarSign, BarChart2, Zap,
} from "lucide-react";
...
export function Activity() {
  ...
  const fetchAll = useCallback(async (d: Days) => {
    setLoading(true);
    try {
      const pulse = await fetch(`${PULSE_URL}?days=${d}`, { signal: AbortSignal.timeout(4000) })
    ...
    <div className="page">
      ...
      <div className="activity-summary-row">
        <div className="activity-summary-item">
          ...
          <span className="activity-summary-value" style={{ color: "var(--success)" }}>
            {fmtMoney(s?.cost_saved_usd)}
          </span>
      ...
      {todayAgents.length > 0 && (
        <ActivitySection label="Today" agents={todayAgents} />
      )}
      {weekAgents.length > 0 && (
        <ActivitySection label="Earlier this week" agents={weekAgents} />
      )}
      ...
      <div className="advanced-section">
        <button className="advanced-toggle" onClick={() => setAdvanced((v) => !v)}>
          ...
          Advanced Stats
          <span className="advanced-toggle-hint">tools, latency, full metrics</span>
        </button>
        ...
        {advanced && (
          <div className="advanced-content">
            <div className="adv-grid">
              ...
              <AdvCard label="Compression ratio" value={s?.compression_ratio != null ? `${s.compression_ratio.toFixed(1)}×` : "—"} />
            </div>
        ...
        <div key={a.agent_id} className={`activity-card ${dimmed ? "activity-card-dimmed" : ""}`}>
          <div className="activity-card-header">
            ...
            <div className="activity-card-meta">
              ...
              {a.last_seen && (
                <span className="activity-card-time">{relTime(a.last_seen)}</span>
              )}
            </div>
          </div>
          <div className="activity-card-stats">
            {a.sessions > 0 && (
              <span className="activity-stat">
                <strong>{a.sessions}</strong> session{a.sessions !== 1 ? "s" : ""}
              </span>
            )}
            {a.tool_calls > 0 && (
              <span className="activity-stat">
                <strong>{fmtNum(a.tool_calls)}</strong> tool calls
              </span>
            )}
            {a.tasks_completed > 0 && (
              <span className="activity-stat activity-stat-success">
                <strong>{a.tasks_completed}</strong> tasks done
              </span>
            )}
            {a.tokens_saved > 0 && (
              <span className="activity-stat activity-stat-accent">
                <strong>{fmtNum(a.tokens_saved)}</strong> context saved
              </span>
            )}
          </div>
        </div>
